...
MAP_REGISTRY = {
    "corridor": dict(
        config="corridor.sumocfg",
        net_file=os.path.join(PROJECT_DIR, "net/corridor/corridor.net.xml")
    ),
    "circular": dict(
        config="circular.sumocfg",
        net_file=os.path.join(PROJECT_DIR, "net/circular/circular.net.xml")
    ),
    "grid": dict(
        config="grid.sumocfg",
        net_file=os.path.join(PROJECT_DIR, "net/grid/grid.net.xml")
    ),
    "manhattan": dict(
        config="manhattan.sumocfg",
...
class TrafficEnv:
    def __init__(
        ...
        checkpoint_path="../logs/checkpoint.xml",
        ...
        self.cav_ids = set()          # vehicles that CAN receive broadcasts
        self.all_managed_ids = set()  # ALL vehicles using pi_react (IDM+MOBIL)
        self._route_pool = []
        self._global_step = 0
    ...
    def start(self):
        ...
        if not self._route_pool:
            raise RuntimeError(
                "No fringe routes found — check net_file / grid geometry"
            )
    ...
    def _discover_fringe_routes(self):
        ...
        fringe_edges = [
            ...
            if (
                len(e.getFromNode().getIncoming())
                + len(e.getFromNode().getOutgoing())
            ) < max_degree
        ]
        ...
        print(
            f"[debug] fringe_edges found: {len(fringe_edges)}, "
            f"routes successfully added: {len(pool)}"
        )
    ...
    def _maybe_spawn(self):
        ...
        # EVERY vehicle uses our driving policy now, connected or not
        disable_native_safety(vid)
        self.all_managed_ids.add(vid)

        # only some vehicles can additionally RECEIVE broadcasts
        if random.random() < self.cav_penetration:
            self.cav_ids.add(vid)

    def register_cav(self, vid):
        # kept for backward compatibility / manual registration (e.g. test scripts)
        if vid not in self.all_managed_ids:
            disable_native_safety(vid)
            self.all_managed_ids.add(vid)
    ...
    def step(self, broadcast_actions):
        ...
        # if communication_enabled=False, `received` stays empty for everyone --
        # every managed vehicle acts purely on its own local LOS (theta = phi)

        for vid in list(self.all_managed_ids):
            if vid not in traci.vehicle.getIDList():
                continue

            broadcast_for_this_vid = (
                received.get(vid, {})
                if vid in self.cav_ids
                else None
            )

            theta = get_known_objects(
                vid,
                broadcast_objects=broadcast_for_this_vid,
                sensing_range=self.sense_range,
                use_occlusion=True
            )
        ...
        self.health.log_ttc_events(traci.vehicle.getIDList())
        ...
        for tx_id in self.cav_ids:
            ...
            local_ids = [
                ...
                if (
                    (tx_x - traci.vehicle.getPosition(v)[0]) ** 2
                    + (tx_y - traci.vehicle.getPosition(v)[1]) ** 2
                ) ** 0.5 <= self.comm_range
            ]

            r, breakdown = self.health.compute_reward(
                tx_id,
                local_ids,
                colliding_ids
            )
